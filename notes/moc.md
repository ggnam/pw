# 목차

## 프로젝트 구성하기

-   [Setup](./setup.md)
-   [Playwright 설정](./playwright/config.md)

## Playwright 기초

-   [기본 구조](./playwright/basic-structure.md)
-   [훅 사용하기](./playwright/hooks.md)
-   [test.skip()과 test.only()로 특정 테스트 실행하기](./playwright/skip-and-only.md)
-   [단언(assertions)으로 값 비교하기](./playwright/assertions.md)
---
-   [스크린샷과 녹화](./playwright/screenshots-and-recordings.md)
-   [소소한 팁들](./playwright/tips.md)

## Playwright 심화
- ... 작성 중

## Playwright 명령어

-   [Commands](./playwright/commands.md)

## 부록

-   [마크다운 사용법](./appendix/markdown.md)
-   [CSS 셀렉터 기초](./appendix/selectors.md)
-   [JavaScript 기초](./appendix/js-basics.md)
